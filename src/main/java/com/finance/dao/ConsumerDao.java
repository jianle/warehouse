...
public class ConsumerDao implements BaseDao<Consumer, Long> {
    ...
    private static final String INSERT_FIELDS = "con_name, account_company, account, taxpayer_id, contact_addr, "
            + "contact_name, contact_tel, remark, insert_dt, update_time";
    ...
    public Boolean save(Consumer object) {
        ...
        try {
            ...
            jdbcTemplateFinance.update(sql, 
                    object.getConName(),
                    object.getAccountCompany(),
                    object.getAccount(),
                    object.getTaxpayerId(),
                    object.getContactAddr(),
                    object.getContactName(),
                    object.getContactTel(),
                    object.getRemark(),
                    object.getInsertDt(),
    ...
    public Boolean update(Consumer object) {
        // TODO Auto-generated method stub
        try {
            String sql = "update " + TABLE_NAME + " set con_name=?, account_company=?, account=?, taxpayer_id=?, contact_addr=?, "
            + "contact_name=?, contact_tel=?, remark=? where con_id=?" ;
            jdbcTemplateFinance.update(sql, 
                    object.getConName(),
                    object.getAccountCompany(),
                    object.getAccount(),
    ...
    public Boolean delete(Long id) {
     ...
     private RowMapper<Consumer> rowMapper = new RowMapper<Consumer>() {
        ...
        public Consumer mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            Consumer.setAccountCompany(rs.getString("account_company"));
            Consumer.setAccount(rs.getString("account"));
            Consumer.setTaxpayerId(rs.getString("taxpayer_id"));
            Consumer.setContactAddr(rs.getString("contact_addr"));
            Consumer.setContactName(rs.getString("contact_name"));
            Consumer.setContactTel(rs.getString("contact_tel"));
            Consumer.setRemark(rs.getString("remark"));
            Consumer.setInsertDt(DATET_TIME_FORMAT.format(rs.getTimestamp("insert_dt")));
