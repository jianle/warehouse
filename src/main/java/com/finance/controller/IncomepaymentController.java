...
public class IncomepaymentController {
    ...
    public JSONArray gettree() {
        List<Incomepayment> incomepayments = incomepaymentDao.findByParentId(0);
        JSONArray jsonArray = new JSONArray();
        
        jsonArray = toJsonArrayByParentId(incomepayments);
        
        return jsonArray;
    }
    
    
    private JSONArray toJsonArrayByParentId(List<Incomepayment> incomepayments) {
        
        JSONArray jsonArray = new JSONArray();
        JSONObject jsonObjectTuple = null;
        for (Incomepayment incomepayment : incomepayments) {
            
            jsonObjectTuple = new JSONObject();
            jsonObjectTuple.put("id", incomepayment.getId());
            jsonObjectTuple.put("text", incomepayment.getType());
            List<Incomepayment> incomepaymentsTuple = incomepaymentDao.findByParentId(incomepayment.getId());
            if (incomepaymentsTuple != null && incomepaymentsTuple.size() > 0) {
                jsonObjectTuple.put("children", toJsonArrayByParentId(incomepaymentsTuple));
            }
